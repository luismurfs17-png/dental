...
export default function PatientDetail() {
  ...
  const loading = patientRemote.loading || recordsRemote.loading || appointmentsRemote.loading || balanceRemote.loading || notesRemote.loading
  const error = patientRemote.error || recordsRemote.error || appointmentsRemote.error || balanceRemote.error || notesRemote.error
  ...
  if (error) return <ErrorState message={error} onRetry={() => { patientRemote.reload(); recordsRemote.reload(); appointmentsRemote.reload(); balanceRemote.reload(); notesRemote.reload() }} />
  ...
  return <><Link className="back-link" to="/pacientes"><Icon name="back" /> Volver al directorio</Link><header className="patient-profile-head"><span className="profile-big-avatar">{name[0]}</span><div><span className="eyebrow">EXPEDIENTE CLÍNICO</span><h1>{name}</h1><p>{patient.codigo} · CI {patient.documento || 'sin registrar'}</p></div><div className="profile-head-actions"><button className="button button-ghost" onClick={() => setEditPatient({ ...patient })}><Icon name="edit" /> Editar</button><button className="button button-coral" onClick={() => setRecordOpen(true)}><Icon name="plus" /> Añadir registro</button>{user.rol === 'doctor' && <button className="icon-button danger" onClick={archivePatient} aria-label="Archivar paciente" title="Archivar paciente"><Icon name="trash" /></button>}</div></header>
    <nav className="patient-contact-strip" aria-label="Acciones de contacto">{patient.telefono && <a href={whatsappUrl(patient.telefono, patient.nombres)} target="_blank" rel="noreferrer"><Icon name="whatsapp" /><span><small>WhatsApp</small><strong>{patient.telefono}</strong></span></a>}{patient.email && <a href={emailUrl(patient.email, patient.nombres)} target="_blank" rel="noreferrer"><Icon name="email" /><span><small>Gmail</small><strong>{patient.email}</strong></span></a>}{patient.telefono && <a href={`tel:${patient.telefono}`}><Icon name="phone" /><span><small>Llamar</small><strong>Contacto directo</strong></span></a>}{!patient.telefono && !patient.email && <span className="no-contact">Sin datos de contacto registrados</span>}</nav>
    ...
    {editPatient && <Modal title="Editar ficha del paciente" onClose={() => setEditPatient(null)}><PatientForm patient={editPatient} setPatient={setEditPatient} onSubmit={savePatient} saving={saving} /></Modal>}<Toast message={message} onClose={() => setMessage('')} /></>
}
