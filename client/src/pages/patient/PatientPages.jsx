...
export function PatientDashboard() {
  ...
  return <><PageHeader eyebrow="TU ESPACIO PERSONAL" title={`Hola, ${(user?.nombre || 'paciente').split(' ')[0]}.`} description="Tu salud dental, clara y siempre a mano." action={<Link className="button button-coral desktop-action" to="/reservar"><Icon name="plus" /> Reservar cita</Link>} />
...
export function PatientPayments() {
  ...
  if ([paymentsRemote, balanceRemote, clinicRemote, profileRemote].some((remote) => remote.loading)) return <Loading label="Consultando tu saldo" />
  const error = paymentsRemote.error || balanceRemote.error || clinicRemote.error || profileRemote.error
  if (error) return <ErrorState message={error} onRetry={() => { paymentsRemote.reload(); balanceRemote.reload(); clinicRemote.reload(); profileRemote.reload() }} />
